...
There is a another component `BpkDividedCard`, which wraps the `BpkCard` compoent with a divider. And it doesn't have padding and also support `href` prop.
...
`BpkDividedCard` could be vertical through the `orientation` prop.
